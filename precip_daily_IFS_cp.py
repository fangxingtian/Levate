...
while yr<=yyyye:
    ...
    while mth<=mthe:
        ...
        while day<=daye:
            ...
            dayarr.append(day)
            #print(data.tp.sel(time=yyyy+'-'+mm+'-'+dd+'T00:00:00.000000000').time)
            day=day+1
            #cidx=cidx+1
        mth=mth+1
    yr=yr+1
...
while cidx<=np.shape(cdatearr)[0]-2:
    ...
    if dayarr[cidx+1]==2 or cidx==0:
        ...
        SHFarray.append(data.tp.sel(time=cdatep+'T00:00:00.000000000').reset_coords('time', drop=True)*1000)
        #print(SHF.max())
    else:
        #print('Use '+cdatep+' - '+cdate)
        SHFarray.append((data.tp.sel(time=cdatep+'T00:00:00.000000000')-data.tp.sel(time=cdate+'T00:00:00.000000000') )*1000)    
        #SHFarray.append(SHF)
    cidx=cidx+1

xarraySHF=xr.concat(SHFarray,dim='time')

xarraySHF.to_netcdf('/scratch/b/b381639/IFS/TP_daily_tco1279-orca025.nc')
